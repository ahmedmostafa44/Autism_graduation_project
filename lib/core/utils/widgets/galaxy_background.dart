...
class _NebulaPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < orbs.length; i++) {
      ...
      final cx = (orb[0]).toDouble() * size.width + drift * size.width;
      final cy = (orb[1]).toDouble() * size.height +
          math.cos((progress + i * 0.2) * math.pi) * 0.03 * size.height;
      final r = (orb[2]).toDouble() * size.width;
      final colorIdx = (orb[3]).toInt() % colors.length;
      final opacity = (orb[4]).toDouble();
